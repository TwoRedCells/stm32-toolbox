...
public:
	...
	void spi_transmit8(SPI_HandleTypeDef spi, uint8_t _data)
	{
		// Envia los 8 mas significativos
		//HAL_SPI_Transmit(&spi, &_data, 1, 100);
		spi_transmit_buf(spi, &_data, 1);
	}


	void spi_transmit16(SPI_HandleTypeDef spi, uint16_t _data)
	{
		// Envia los 8 mas significativos
		//HAL_SPI_Transmit(&spi, (uint8_t *)((_data & 0xFF00) >> 8), 1, 100);
		spi_transmit8(spi, (uint8_t)((_data >> 8) & 0xFF));
		// Envia los 8 menos significativos
		//HAL_SPI_Transmit(&spi, (uint8_t *)((_data & 0x00FF)), 1, 100);
		spi_transmit8(spi, (uint8_t)(_data & 0xFF));
	}


	void spi_transmit_buf(SPI_HandleTypeDef spi, uint8_t *_data, uint16_t _len)
	{
	...
	uint8_t spi_receive8(SPI_HandleTypeDef spi)
	{
		...
		spi_receive_buf(spi, &dato_recibido, 1);
	...
	void spi_receive_buf(SPI_HandleTypeDef spi, uint8_t *dato_recibido, uint16_t _len)
	{
		for(int i = 0; i < _len; i++) {
	...
	void recv_data_processing(SOCKET s, uint8_t *data, uint16_t len, uint8_t peek) 
	{
